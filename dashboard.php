...
$queryMonth = "SELECT count(*) as `month` 
  ...
  try {
    ...
    $stmtToday = DBContext::getDB()->prepare($queryToday);
    $stmtToday->execute();
    $r = $stmtToday->fetch(PDO::FETCH_ASSOC);
    $todaysCount = $r['today'];

    $stmtWeek = DBContext::getDB()->prepare($queryWeek);
    $stmtWeek->execute();
    $r = $stmtWeek->fetch(PDO::FETCH_ASSOC);
    $currentWeekCount = $r['week'];

    $stmtYear = DBContext::getDB()->prepare($queryYear);
    $stmtYear->execute();
    $r = $stmtYear->fetch(PDO::FETCH_ASSOC);
    $currentYearCount = $r['year'];

    $stmtMonth = DBContext::getDB()->prepare($queryMonth);
    $stmtMonth->execute();
